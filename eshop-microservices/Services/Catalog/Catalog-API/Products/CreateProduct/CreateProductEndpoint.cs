    ...
    public class CreateProductEndpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            app.MapPost("/products", async (CreateProductRequest req, ISender sender) =>
            {
                ...
                var result = await sender.Send(command); //sends to the handler
                var response = result.Adapt<CreateProductResponse>();  //adapt the response from the handler to the body
